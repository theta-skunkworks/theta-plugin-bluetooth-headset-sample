...
public class ChangeEvTask extends AsyncTask<Void, Void, Integer> {
    private static final String TAG = "ChangeEvTask";
    ...
    synchronized protected Integer doInBackground(Void... params) {
        ...
        try {
            ...
            if (captureStatus.equals("idle") && (recordedTime == 0)) {
                ...
                strResult = camera.httpExec(HttpConnector.HTTP_POST, HttpConnector.API_URL_CMD_EXEC,
                        strJsonGetCaptureMode);
                ...
                if (curExposureProgram != 1) {    //not MANUAL
                    ...
                    String strJsonSetCaptureMode =
                            "{\"name\": \"camera.setOptions\", \"parameters\": { \"options\":{\"exposureCompensation\":\""
                                    + evList[setEv] + "\"} } }";
                    strResult = camera
                            .httpExec(HttpConnector.HTTP_POST, HttpConnector.API_URL_CMD_EXEC,
                                    strJsonSetCaptureMode);
